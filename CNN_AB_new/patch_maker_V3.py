...
def make_patch(event_hits, bx, by, bz, patch_size, source_label):
    """
    Enhanced patch with 6 channels:
    - Channel 0: Layer A energy
    - Channel 1: Layer B energy
    - Channel 2: Layer A multiplicity
    - Channel 3: Layer B multiplicity
    - Channel 4: Layer A spatial extent (z and phi std)
    - Channel 5: Layer B spatial extent (z and phi std)
    """
    ...
    # 6 channels: energy(2) + multiplicity(2) + extent(2)
    patch = np.zeros((6, patch_size, patch_size), dtype=np.float32)
    patch_hits = []
    
    # Store positions for spatial extent calculation
    z_positions = defaultdict(list)
    phi_positions = defaultdict(list)
    ...
    for h in event_hits:
        ...
        # Channel 0-1: Energy deposition
        patch[layer, iz, iphi] += h.edep

        # Channel 2-3: Multiplicity
        patch[2 + layer, iz, iphi] += 1

        # Store positions for extent calculation
        key = (layer, iz, iphi)
        z_positions[key].append(h.z)
        phi_positions[key].append(math.atan2(h.y, h.x))
    ...
    # Calculate spatial extent (standard deviation of z and phi)
    for (layer, iz, iphi), z_list in z_positions.items():
        if len(z_list) > 1:
            # Channel 4-5: Combined z and phi extent
            z_std = np.std(z_list)
            phi_list = phi_positions[(layer, iz, iphi)]
            
            # Handle phi wraparound (circular mean)
            phi_std = np.std(phi_list) if len(phi_list) > 1 else 0.0
            
            # Combine z and phi extent (you can also use separate channels)
            patch[4 + layer, iz, iphi] = z_std + phi_std
...
def main():
    ...
    parser.add_argument("--out", type=str, default="/ceph/submit/data/user/h/haoyun22/Patches_CNN_Data/AB_patches_V3.npz")
    ...
    for sname, cfg in SAMPLES.items():
        ...
        BAD_FILES = {
            # "/ceph/submit/data/group/fcc/ee/detector/VTXStudiesFullSim/CLD_wz3p6_ee_qq_ecm91p2/proc_11_3.root",
